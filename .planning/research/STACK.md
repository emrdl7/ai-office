...
**Domain:** Local AI multi-agent collaboration system with web dashboard
**Researched:** 2026-04-03
**Confidence:** MEDIUM-HIGH (core stack verified via official sources; some version details from PyPI/Ollama docs)
...
```bash
# Python 환경 (uv 사용 권장)
uv init ai-office-backend
cd ai-office-backend
uv add fastapi uvicorn[standard] crewai crewai-tools ollama sqlmodel pydantic-settings httpx python-multipart

# 개발 의존성
uv add --dev ruff pytest pytest-asyncio httpx

# React 대시보드 (Vite 스캐폴드)
npm create vite@latest ai-office-dashboard -- --template react-ts
cd ai-office-dashboard
npm install react-use-websocket zustand @tanstack/react-query tailwindcss @tailwindcss/vite @monaco-editor/react
...
```
Claude Agent SDK (`claude-agent-sdk` PyPI)도 존재하나, SDK는 내부적으로 동일한 subprocess 패턴을 사용. 직접 subprocess 제어가 더 투명함.
...
*Stack research for: AI Office — local multi-agent collaboration system*
*Researched: 2026-04-03*
