...
async def test_claude_analyzes_user_instruction(loop_setup):
    ...
    with patch(
        "orchestration.loop.run_claude_isolated", new_callable=AsyncMock
    ) as mock_claude:
...
async def test_memory_inject_on_run_agent(loop_setup, tmp_path):
    ...
    runner.generate_json = AsyncMock(
        return_value={
            "status": "success",
            "artifact_paths": [],
            "summary": "작업 완료",
    ...
    assert "## 이전 경험" in system_arg, (
        f"system_prompt에 이전 경험이 없음: {system_arg!r}"
    )
