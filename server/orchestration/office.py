...
class Office:
  ...
  async def _handle_quick_task(
    ...
    ctx_parts = [
      ...
      f'[중요: 산출물 작성 규칙]\n'
      f'- "확인했습니다", "파악하겠습니다", "작성하겠습니다" 같은 착수 선언은 금지.\n'
      f'- 이 메시지에 대한 응답이 곧 최종 산출물이다. 바로 본문을 작성하라.\n'
      f'- 분석, 리뷰, 기획 등 요청받은 결과물을 즉시 완성된 형태로 출력하라.'
    ]
    if self._context_summary:
  ...
  async def _execute_project(
    ...
    for phase in PHASES:
      ...
      phase_prompt += (
        ...
        f'"확인했습니다", "작성하겠습니다" 같은 착수 선언 없이 바로 본문을 작성하라.'
      )
