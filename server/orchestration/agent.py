...
class Agent:
  ...
  async def reflect(self, topic: str) -> str:
    ...
    prompt = (
      ...
      f'1. **빈 칭찬/응원/감탄 금지**: "멋져요", "좋네요", "화이팅", "기대됩니다", '
      f'"천만에요", "감사합니다" 같은 내용 없는 말은 절대 금지.\n'
      f'2. **가상 제안 칭찬 금지**: 상대가 실제로 제안한 게 없으면 "제안 좋아요"라고 하지 마라.\n'
      f'3. **구체성 필수**: 파일명, 수치, 구체 기법, 실제 경험 사례 중 하나 이상 포함.\n\n'
      f'[발언할 만한 경우 — 이 중 해당될 때만]\n'
      f'A. 구체적 아이디어나 제안 (예: "IA 설계 때 모바일 먼저 보는 게 속도가 빠릅니다")\n'
      f'B. 실제 기술/업계 정보 공유 (예: "최근 Tailwind v4 나왔는데 JIT가 더 빨라졌어요")\n'
      f'C. 문제 제기/개선 필요 사항 (예: "QA 불합격 패턴이 최근 기획 단계에 몰려있네요")\n'
      f'D. 동료 작업에 대한 **구체적** 피드백 (근거 있는 지적/대안)\n'
      f'E. 질문/토의 주제 제시 (예: "우리 프로젝트 QA 기준 너무 엄격한가요?")\n'
      f'F. 가벼운 잡담이지만 화제성 있는 내용 (특정 경험/관찰, "오늘 OO 기사 봤는데...")\n\n'
      f'[출력]\n'
      f'- 위 A-F 중 하나에 해당하면 1~2문장으로 말하세요.\n'
      f'- 해당 안 되면 [PASS]. 70% 확률로는 [PASS]가 정답이다. 침묵이 빈 칭찬보다 낫다.\n'
      f'- 메신저 톤, 마크다운 금지.'
    )
